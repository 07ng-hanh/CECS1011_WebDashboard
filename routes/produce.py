...
async def list_produces(page: int = 1, limit: int = 20, query: str = "", pgpool: asyncpg.pool.Pool = Depends(get_pgpool)):
    async with pgpool.acquire() as con:
        try:
            ...
            produces = await con.fetch("select id, harvest_type_name, shelf_life, thresh_temp_lo, thresh_temp_hi, thresh_humidity_lo, thresh_humidity_hi, thresh_co2_lo, thresh_co2_hi from produceinfo where harvest_type_name ilike $1 limit $2 offset $3",
